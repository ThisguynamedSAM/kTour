...
public class kTour extends JFrame {
    ...
    private static int[][] testBoard = new int[dim][dim]; // this is a temporary board size 8X8
    // will be removed after object -> int[][] problem is solved
    private static int[][] access = new int[dim][dim]; // accessibility matrix
    ...
    kTour() {
        //    Things to add
        // create object board b
        // use b.temp instead of testboard
        // add look ahead method for access matrix
        // create new constructor that will iterate through each starting position and complete
        // the tour
    ...
    private static boolean isLegal(int move[]) {
        ...
        if(( move[0] >= 0 && move[0] < dim ) && ( move[1] >= 0 && move[1] < dim )) { // within bounds?
